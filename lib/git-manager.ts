...
export default {
   ...
   getOpenPulls: function(repo) {
      ...
      // TODO: This might need to be 'list' instead
      return github.pullRequests.list(params({}, repo)).then(paginate);
   },
   ...
   getAllPulls: function(repo) {
      ...
      return github.pullRequests.list(params({ state: 'all' }, repo))
                                .then(paginate);
   },
   ...
   getOpenIssues: function(repo) {
      ...
      return github.issues.list(searchParams)
      .then(paginate)
      .then(filterOutPulls)
      .then(addRepo(searchParams));
   },
   ...
   parse: function(githubPull) {
      ...
      return Bluebird.all([reviewComments,
                          comments,
                          headCommit,
                          commitStatus,
                          events,
...
function getCommitStatus(repo, ref) {
   ...
   return github.repos.listStatusesForRef(params({ ref }, repo))
                      .then(res => res.data)
